...
class DataLoader(object):
    ...
    def preprocess_for_predict(self):
        ...
        for d, user in zip(self.test_data, self.test_user):  # the pad is needed! but to be careful.
            ...
            processed.append(
                [seq, xd, yd, [d[2]], position, x_position, y_position, user, x_flag, y_flag, negative_sample])
    ...
    def preprocess(self):
        ...
        for d, user in zip(self.train_data, self.train_user):  # the pad is needed! but to be careful.
            ...
            processed.append(
                [d, xd, yd, [g], [xg], [yg], position, x_position, y_position, user, x_flag, y_flag, negative_sample,
                 x_negative_sample,
                 y_negative_sample])
