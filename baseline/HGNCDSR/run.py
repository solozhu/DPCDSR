...
def evaluation(model, test_data_x, test_data_y, topk=10):
    ...
    for batch in test_data_x:   # ground truth belongs to x domain
        ...
        prediction_score = model(seq, x_seq, y_seq, position, x_position, y_position, user, items_to_predict, for_pred=True, pred_domain='x')
    ...
    for batch in test_data_y:   # ground truth belongs to x domain, 同理
        ...
        prediction_score = model(seq, x_seq, y_seq, position, x_position, y_position, user, items_to_predict, for_pred=True, pred_domain='y')
        prediction_score = prediction_score.cpu().data.numpy().copy()
...
def train_model(model, optim, train_data, valid_data_x, valid_data_y, test_data_x, test_data_y, conf):
    ...
    for epoch_num in range(conf.n_iter):
        ...
        for batch in train_data:
            ...
            prediction_score, x_prediction_score, y_prediction_score = model(seq, x_seq, y_seq, position, x_position, y_position, user, items_to_predict,x_items_to_predict, y_items_to_predict, False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='Entertainment-Education', help='Food-Kitchen, Movie-Book, Entertainment-Education')                                           # domain
