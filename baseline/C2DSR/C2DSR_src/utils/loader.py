...
class DataLoader(object):
    ...
    def __init__(self, filename, batch_size, opt, evaluation):
        ...
        source_valid_data = "./dataset/" + filename + "/validdata_new2.txt"
        source_test_data = "./dataset/" + filename + "/testdata_new2.txt"
        if not os.path.exists(source_valid_data):
            source_valid_data = "./dataset/" + filename + "/validdata_new.txt"
        if not os.path.exists(source_test_data):
    ...
    def preprocess_for_predict(self):
        ...
        for d in self.test_data: # the pad is needed! but to be careful.
            ...
            # The HGNCDSR EE split contains many repeated target items in the
            # prefix.  Remove every occurrence of the held-out target from the
            # model input so evaluation cannot reuse the answer from history.
            history = [item for item in d[0] if item != d[2]][-max_len:]
            ...
            pad_num = max_len - len(history)
            pad_item = self.opt["source_item_num"] + self.opt["target_item_num"]
            xd = [pad_item] * pad_num + xd
            yd = [pad_item] * pad_num + yd
            seq = [pad_item] * pad_num + history

            # Match the HGNCDSR evaluation interface: when predicting one
            # domain, hide the most recent item from the paired domain path.
            if d[1]:
            ...
            for i in range(999):
                while True:
                    if d[1] : # in Y domain, the validation/test negative samples
                        ...
                        global_sample = sample + self.opt["source_item_num"]
                        if global_sample not in seen_items:
