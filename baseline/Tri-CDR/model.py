...
class NTXentLoss(torch.nn.Module):#涓嶅姞鏍囩淇℃伅鐨刲oss
...
#         ipdb.set_trace()
        if self.fname == 'amazon_game':
            ...
            dev = target_log_feats_sas.device
...
#                     ipdb.set_trace()
                    ...
                    mix_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, i, :], mix_log_feats_sas, matrix_mix.to(dev), mix_log_feats_sas, matrix_mix.to(dev))
                    source_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, i, :], source_log_feats_sas, matrix_source.to(dev), mix_log_feats_sas, matrix_mix.to(dev))
        
                    target_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, i, :], target_log_feats_sas[:, 0:i+1, :], torch.as_tensor(target_log_seqs[:, 0:i+1], device=dev), mix_log_feats_sas, matrix_mix.to(dev))
...
#                     ipdb.set_trace()
                    ...
                    mix_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, i, :], mix_log_feats_sas, matrix_mix.to(dev), mix_log_feats_sas, matrix_mix.to(dev))
                    source_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, i, :], source_log_feats_sas, matrix_source.to(dev), mix_log_feats_sas, matrix_mix.to(dev))
                    target_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, i, :], target_log_feats_sas[:, start_index_target:i+1, :], torch.as_tensor(target_log_seqs[:, start_index_target:i+1], device=dev), mix_log_feats_sas, matrix_mix.to(dev))
                    mix_log_feats = torch.cat([mix_log_feats, mix_log_feats_final_dim.unsqueeze(1)], dim=1)
                    source_log_feats = torch.cat([source_log_feats, source_log_feats_final_dim.unsqueeze(1)], dim=1)
                    target_log_feats = torch.cat([target_log_feats, target_log_feats_final_dim.unsqueeze(1)], dim=1)
...
#             # Attention for the last dimension
            ...
            mix_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, position-1, :], mix_log_feats_sas, torch.as_tensor(mix_log_seqs, device=dev), mix_log_feats_sas, torch.as_tensor(mix_log_seqs, device=dev))
                # Attention in the source domain for the last dimension
            source_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, position-1, :], source_log_feats_sas, torch.as_tensor(source_log_seqs, device=dev), mix_log_feats_sas, torch.as_tensor(mix_log_seqs, device=dev))
                # Attention in the target domain for the last dimension
            target_log_feats_final_dim = self.DINAttentionLayer_withmixattn(target_log_feats_sas[:, position-1, :], target_log_feats_sas, torch.as_tensor(target_log_seqs, device=dev), mix_log_feats_sas, torch.as_tensor(mix_log_seqs, device=dev))
