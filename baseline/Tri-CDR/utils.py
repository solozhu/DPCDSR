...
def sample_function(version, fname, crossdataset, interval, user_train_mix, user_train_source, user_train_target, user_train_mix_sequence_for_target, user_train_source_sequence_for_target, usernum, itemnum, batch_size, maxlen, result_queue, SEED):        
    ...
    def sample():
        ...
        nxt_target = user_train_target[user][-1] # # 鏈€鍚庝竴涓氦浜掔殑鐗╁搧
        ...
        for i in reversed(range(0, len(user_train_mix[user]))): # reversed鏄€嗗簭鎼滅储锛岃繖閲岀殑i鎸囩殑鏄氦浜掔殑鐗╁搧
            seq_mix[idx_mix] = user_train_mix[user][i]
            idx_mix -= 1
            if idx_mix == -1: break
                
        for i in reversed(range(0, len(user_train_source[user]))): # reversed鏄€嗗簭鎼滅储锛岃繖閲岀殑i鎸囩殑鏄氦浜掔殑鐗╁搧
            seq_source[idx_source] = user_train_source[user][i]
            idx_source -= 1
            if idx_source == -1: break
                
        for i in reversed(range(0, len(user_train_target[user][:-1]))): # reversed鏄€嗗簭鎼滅储锛岃繖閲岀殑i鎸囩殑鏄氦浜掔殑鐗╁搧
...
def data_partition(version, fname, dataset_name, maxlen):
    if version == 'GradProj':
        root = os.environ.get('GRADPROJ_DATA_ROOT')
        if root is None:
            root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'DiffCDSR1', 'dataset'))
        if fname == 'amazon_toy':
            target_domain = 'x'
        elif fname == 'amazon_game':
            target_domain = 'y'
        else:
            raise ValueError("GradProj Tri-CDR expects --dataset amazon_toy for x target or amazon_game for y target")
        return GradProjCDSR(root, dataset_name).tricdr_partition(target_domain)
...
def evaluate_SASRec(model, dataset, args, split='test'):
    with torch.no_grad():
        ...
        valid_prefix = dataset[10] if len(dataset) > 10 else {}
        test_prefix = dataset[11] if len(dataset) > 11 else {}
        ...
        for u in users:
            if len(user_train_mix[u]) < 1 or len(user_train_source[u]) < 1 or len(user_train_target[u]) < 1:
                continue
            if split == 'valid' and len(user_valid_target[u]) < 1:
                continue
            if split == 'test' and len(user_test_target[u]) < 1:
                continue
            ...
            if split == 'valid' and u in valid_prefix and valid_prefix[u][0]:
                mix_prefix, source_prefix, target_prefix = valid_prefix[u]
            elif split == 'test' and u in test_prefix and test_prefix[u][0]:
                mix_prefix, source_prefix, target_prefix = test_prefix[u]
            else:
                mix_prefix, source_prefix, target_prefix = user_train_mix[u], user_train_source[u], user_train_target[u]

            for i in reversed(mix_prefix): # reversed鏄€嗗簭鎼滅储锛岃繖閲岀殑i鎸囩殑鏄氦浜掔殑鐗╁搧
                seq_mix[idx_mix] = i
                idx_mix -= 1
                if idx_mix == -1: break

            for i in reversed(source_prefix): # reversed鏄€嗗簭鎼滅储锛岃繖閲岀殑i鎸囩殑鏄氦浜掔殑鐗╁搧
                seq_source[idx_source] = i
                idx_source -= 1
                if idx_source == -1: break

            for i in reversed(target_prefix): # reversed鏄€嗗簭鎼滅储锛岃繖閲岀殑i鎸囩殑鏄氦浜掔殑鐗╁搧
            ...
            item_idx[0] = user_valid_target[u][0] if split == 'valid' else user_test_target[u][0]
...
#             ipdb.set_trace()
    ...
    return {
        ...
        "HR@1": HT_1 / valid_user,
        "HR@5": HT_5 / valid_user,
        "HR@10": HT_10 / valid_user,
    }
